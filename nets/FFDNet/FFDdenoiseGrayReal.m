    function FFDdenoiseGrayReal(path,imageSet,useGPU)
    ...
    folderResultCur       =  fullfile(path, ['results\FFDnet_grayReal_',imageSet,'_REAL_',datestr(now,'mm_dd_HH-MM-SS')]);
    ...
    for i = 1:length(filePaths)
        ...
        label = imread(fullfile(path,folderTest,setTestCur,filePaths(i).name));
        [w,h,~]=size(label);
        if size(label,3)==3
