...
mainpath = 'A:\PROJECTS\2019_11_AIP\DnCNN\matconvnet-master\matconvnet-master\FFDNet'

format compact;
global sigmas; % input noise level or input noise level map
addpath(fullfile(mainpath,'utilities'));
...
folderResultCur       =  fullfile(mainpath,folderResult, [setTestCur,'_',num2str(imageNoiseSigma(1)),'_',num2str(inputNoiseSigma(1))]);
...
filePaths   =  [];
for i = 1 : length(ext)
    filePaths = cat(1,filePaths, dir(fullfile(mainpath,folderTest,setTestCur,ext{i})));
end
...
for i = 1:length(filePaths)
    ...
    if c == 3
        ...
        if showResult
            ...
            imwrite(im2uint8(vertcat(input,output)),fullfile(folderResultCur, [nameCur, '_' num2str(imageNoiseSigma(1),'%02d'),'_compare' num2str(inputNoiseSigma(1),'%02d'),'_PSNR_',num2str(PSNRCur*100,'%4.0f'), extCur]));
