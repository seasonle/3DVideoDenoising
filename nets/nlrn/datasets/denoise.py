...
def test_saved_model():
    ...
    parser.add_argument(
        '--input-dir', help='GCS location to load input images', required=False)
    parser.add_argument(
    ...
    with tf.compat.v1.Session(graph=tf.Graph()) as sess:
        metagraph_def = tf.compat.v1.saved_model.loader.load(
            sess, [tf.compat.v1.saved_model.tag_constants.SERVING], args.model_dir)
        ...
        for i in range(len(data3)):
            ...
            sha = sha256(str(random()).encode('utf-8'));
            seed = np.frombuffer(sha.digest(), dtype='uint32')
            rstate = np.random.RandomState(seed)

            output_file = os.path.join(args.output_dir, str(i))

            input_image = np.asarray(data3[i]);
